...
const { PlaylistService } = require("./modules/playlist");
...
const services = [databaseService, authService, pollService, sessionService, playlistService];
...
SERVER.TIME = 0 - SERVER.settings.vc.head_time; // referring to video time
SERVER._TIME = new Date().getTime(); // Previous tick time.
SERVER.OUTBUFFER = {};
SERVER.BANS = [];
SERVER.FILTERS = [];
SERVER.DRINKS = 0;
...
const ticker = setInterval(() => {
	...
	for (const service of services) {
		service.onTick(timestamp - SERVER._TIME);
	}
