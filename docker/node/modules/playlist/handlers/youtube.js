...
exports.YoutubeHandler = class extends Handler {
	...
	async getGeoblockedCountries(restrictions) {
		if (!restrictions.blocked && !restrictions.allowed) {
			return false;
		}
	...
	async handle(data) {
		...
		const restrictions = {
			...
			geoblock: data.force ? false : this.getGeoblockedCountries(video.contentDetails.regionRestriction),
